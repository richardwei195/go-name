...
// 计算五行wx_b对wx_a是否有正向作用
func PositiveRelation(wx_a string, wx_b string) bool {
...
func ComputeDestiny(sd *SolarTianGanDiZhi) {

	score := 0
	riyuan := sd.DtgWX
	if PositiveRelation(riyuan, sd.MdzWX) {
		score = score + 35
	}
	if PositiveRelation(riyuan, sd.MtgWX) {
		score = score + 14
	}
	if PositiveRelation(riyuan, sd.DdzWX) {
		score = score + 14
	}
	if PositiveRelation(riyuan, sd.HtgWX) {
		score = score + 14
	}
	if PositiveRelation(riyuan, sd.HdzWX) {
		score = score + 8
	}
	if PositiveRelation(riyuan, sd.YtgWX) {
		score = score + 10
	}
	if PositiveRelation(riyuan, sd.YdzWX) {
		score = score + 5
	}
	...
	sd.DestinyScore = score
	sd.DestinyLevel = destinylevel
}

func ComputePositiveGod(sd *SolarTianGanDiZhi, score int) {
...
func (sc *Calendar) tianGanDiZhi(sd *SolarDate) {
	...
	/* add by collin*/
	sdgz.YtgWX = TianGanWuXing(sdgz.Ytg)
	sdgz.YdzWX = DiZhiWuXing(sdgz.Ydz)
	sdgz.MtgWX = TianGanWuXing(sdgz.Mtg)
	sdgz.MdzWX = DiZhiWuXing(sdgz.Mdz)
	sdgz.DtgWX = TianGanWuXing(sdgz.Dtg)
	sdgz.DdzWX = DiZhiWuXing(sdgz.Ddz)
	sdgz.HtgWX = TianGanWuXing(sdgz.Htg)
	sdgz.HdzWX = DiZhiWuXing(sdgz.Hdz)

	ComputeDestiny(sdgz)
	ComputePositiveGod(sdgz, sdgz.DestinyScore)
...
func TestCalendar() int {
	...
	//	fmt.Printf("公历%d年%d月%d日转成农历是: %d(%s%s)[%s]年%s%s月%s\n",d.Year,d.Month,d.Day,ld.Year,ld.YearGanZi.Gan,ld.YearGanZi.Zhi,ld.YearGanZi.Animal, ld.LeapStr, ld.MonthStr, ld.DayStr)

	//	ssd,err := sc.Lunar2Solar(ld.Year,ld.Month,ld.Day,ld.LeapMonth)
	//	if err != nil {
	//		fmt.Println("err")
	//	}
	//	fmt.Printf("农历%d年%d月%d日转成公历是: %d年%d月%d日\n",ld.Year,ld.Month,ld.Day,ssd.Year,ssd.Month,ssd.Day)
	fmt.Printf("出生时间:%d年%d月%d日%d时，农历: %s%s年%s%s月%s%s日%s%s时\n", d.Year, d.Month, d.Day, d.Hour, ssd.GanZhi.YtgStr, ssd.GanZhi.YdzStr, ssd.GanZhi.MtgStr, ssd.GanZhi.MdzStr, ssd.GanZhi.DtgStr, ssd.GanZhi.DdzStr, ssd.GanZhi.HtgStr, ssd.GanZhi.HdzStr)
	fmt.Printf("八字五行:年:%s%s\t月:%s%s\t日:%s%s\t时:%s%s\n", ssd.GanZhi.YtgWX, ssd.GanZhi.YdzWX, ssd.GanZhi.MtgWX, ssd.GanZhi.MdzWX, ssd.GanZhi.DtgWX, ssd.GanZhi.DdzWX, ssd.GanZhi.HtgWX, ssd.GanZhi.HdzWX)
	fmt.Printf("八字得分:%d\t八字%s\n", ssd.GanZhi.DestinyScore, ssd.GanZhi.DestinyLevel)
	fmt.Printf("喜神用神:%s\n", ssd.GanZhi.PositiveGod)
	return d.Year
