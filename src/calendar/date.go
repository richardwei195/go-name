...
// YmdhNewDate 指定年月日小时初始SolarDate
func YmdhNewDate(y,m,d,h int,loc *time.Location)*Date{
	if loc == nil {
		loc = time.Local
	}
	t := time.Date(y,time.Month(m),d,h,1,0,0,loc)
	return TimeToDate(t)
}

// YmdhNewDate 指定年月日小时初始SolarDate
func YmdhmNewDate(y,m,d,h,mm int,loc *time.Location)*Date{
	if loc == nil {
		loc = time.Local
	}
